...
PPO_DEFAULT_CONFIG = {
    ...
    "mini_batches": 16,              # number of mini batches during each learning epoch 
...
class PPO(Agent):
    ...
    def record_transition(
        ...
        if self.memory is not None:
            ...
            # 올바른 구현: V(s_{t+1}) 사용 (next state의 value로 bootstrap)
            # V(s_t) 사용은 잘못된 bootstrap — reward에 (γ-1)*V(s_t) 추가가 됨
            if self._time_limit_bootstrap:
                ...
                rewards += self._discount_factor * next_values * truncated
    ...
    def _update(self, timestep: int, timesteps: int) -> None:
        ...
        with torch.no_grad(), torch.autocast(device_type=self._device_type, enabled=self._mixed_precision):
            ...
            assert self._current_next_states is not None, "next_states cannot be None during update"
        ...
        returns, advantages = compute_gae(
            ...
            # time_limit_bootstrap=True 일 때:
            #   truncated step의 rewards에 γ*V(s_{t+1})가 이미 더해져 있음.
            #   따라서 GAE에서도 truncated를 terminal로 취급해야(dones=1) lambda 항이 끊겨 새 에피소드의 advantage가 이전 에피소드로 역전파되지 않음.
            ...
            for (
                ...
                with torch.autocast(device_type=self._device_type, enabled=self._mixed_precision):
                    ...
                    if self._entropy_loss_scale:
                        ...
                        entropy_loss = torch.tensor(0.0, device=self._device_type)
